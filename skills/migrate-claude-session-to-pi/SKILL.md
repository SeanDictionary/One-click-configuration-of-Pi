...
- `<target-cwd>` — the working directory the pi session should bind to (usually the original project dir). This also determines the output directory.
- `--cutoff` — ISO timestamp. Turns at/after it are kept as a full recent conversation; earlier turns are condensed into one structured **summary** user message (project context, user-prompt timeline, files-touched list, key progress, last status). Omit or pass `""` to keep **all** turns as recent (no summary).

Output is written to pi's default session directory for `<target-cwd>`: `~/.pi/agent/sessions/--<encoded-cwd>--/<ISO>_<uuid>.jsonl`, so `pi --resume` in that directory will list it automatically.

### Example

```bash
python claude2pi.py \
  ~/.claude/projects/-home-me-myproj/abc-123.jsonl \
  /home/me/myproj \
...
| `cwd` (positional, required) | — | target working directory (also selects the output dir) |
...
| `--out-dir` | pi default dir for `cwd` | override the output directory |
| `--language` | `en` | summary text language: `en` or `zh` (does not affect migrated conversation content) |
...
- **Output directory**: mirrors pi's own encoding — `--` + (cwd with leading separator stripped, then `/ \ :` replaced by `-`) + `--`, under `~/.pi/agent/sessions/`. Use `--out-dir` to override.
